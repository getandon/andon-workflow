...
import { TEMPORAL_DEFAULT_ADDRESS, buildTemporalTls, temporalNamespace } from '../../../../libs/common/src';
...
export class TemporalWorkerService implements OnModuleInit, OnModuleDestroy {
  ...
  private async registerWorker(taskQueue: string, environment: string, activities: string[]) {
    try {
      ...
      if (!res.ok) {
        console.warn(`Worker registration failed (HTTP ${res.status}): ${await res.text()}`);
      }
    } catch (err) {
  ...
  private async sendHeartbeat() {
    try {
      const res = await fetch(`${API_URL}/api/workers/${encodeURIComponent(this.workerName)}/heartbeat`, {
        method: 'POST',
        headers: authHeaders(),
      });
      if (!res.ok) {
